...
class ExpenseByCategory(GridLayout):

    def __init__(self, month, data, **kwargs):
        super(ExpenseByCategory, self).__init__(**kwargs)
        self.cols = 1
        data = data
        expenses = data['expenses'][month]
        expense_keys = expenses.keys()
        ordered = sorted(expenses.items(), key = lambda x: x[1]['category'])
        ...
        for i in range(len(ordered)):
            ...
            if category_name in category_dict.keys():
                category_dict[category_name] += ordered[i][1]['price']
            else:
                category_dict[category_name] = ordered[i][1]['price']

        for k in category_dict:
            x = k
            i = 'icons/expenses/' + x.lower() + '.png'
            # i = 'icons/expenses/pet.png'
            pNum = category_dict[k]
            pFloat = '%.2f' % pNum
            p = '$ ' + pFloat
            pTotal += pNum
            # widgets
            ...
            expenseFloat.add_widget(Label(text=x, text_size=self.size, size_hint=(.5, 1),
                                          pos_hint={"x": .125, "y": 1}, halign='left'))
